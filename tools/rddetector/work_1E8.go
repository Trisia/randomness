...
const Header_1E8 = "源数据," +
	...
	"[ 4] 重叠子序列检测 m=7 P1,重叠子序列检测 m=7 P2," +
	"[ 5] 游程总数检测," +
	"[ 6] 游程分布检测," +
	"[ 7] 块内最大游程检测 m=10000," +
	"[ 8] 二元推导检测 k=3," +
...
func worker_1E8(jobs <-chan string, out chan<- *R) {
	for filename := range jobs {
		...
		// [2] 块内频数检测
		p, _ = randomness.FrequencyWithinBlockProto(bits, 100_000)
		arr = append(arr, p)
		...
		arr = append(arr, p1, p2)
		log.Printf("[%s] 重叠子序列检测 m=7 P1: %.5f P2: %.5f", filename, p1, p2)
		...
		arr = append(arr, p)
		log.Printf("[%s] 二元推导检测 m=15 P: %.5f", filename, p)
		...
		arr = append(arr, p)
		log.Printf("[%s] 自相关检测 m=32 P: %.5f", filename, p)
		...
		arr = append(arr, p)
		log.Printf("[%s] 累加和检测 前向 P: %.5f", filename, p)
		p, _ = randomness.CumulativeTest(bits, false)
		arr = append(arr, p)
		log.Printf("[%s] 累加和检测 后向 P: %.5f", filename, p)
		...
		arr = append(arr, p)
		log.Printf("[%s] 近似熵检测 m=7 P: %.5f", filename, p)

		// [13] 线性复杂度检测
		p, _ = randomness.LinearComplexityProto(bits, 5_000)
		arr = append(arr, p)
		log.Printf("[%s] 线性复杂度检测 m=5000 P: %.5f", filename, p)
