...
// worker_1E8 检测数据规模为 100 000 000 bit 的随机数列。
func worker_1E8(jobs <-chan string, out chan<- *R) {
	for filename := range jobs {
		...
		// [2] 块内频数检测
		p, q = randomness.FrequencyWithinBlockTestBitSeq(bits, 100000)
		...
		p1, p2, q1, q2 = randomness.OverlappingTemplateMatchingTestBitSeq(bits, 7)
		...
		p, q = randomness.BinaryDerivativeTestBitSeq(bits, 15)
